<?php
	...
	$errors = array('title' => '', 'startdate' => '' , 'enddate' => '' ,'cinemaname' => '', 'category' => '');
	...
	if(isset($_POST['submit'])){
		...
		if(!preg_match('/^[\w\-\s]+$/', $title)){
			$errors['title'] = 'Title must be letters,numbers,dashes and spaces only';
		}
		...
		if(!preg_match('/^[\w\-\s]+$/', $cinemaname)){
			$errors['cinemaname'] = 'Cinema name must be letters,numbers,dashes and spaces only';
		}
		...
		// If not exist errors then update the movies and cinemas table 
		if(!array_filter($errors)){

			// Update selected movie on movies table
			$curl = curl_init();
			...
			curl_setopt_array($curl, array(
				CURLOPT_URL => 'http://172.18.1.10:1027/api/movies/'.$_POST['id'],
			...
			if (strcmp($httpcode,'200') == 0) { $print_message = "Record updated successfully"; }
